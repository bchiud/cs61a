...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        "*** YOUR CODE HERE ***"
        pressed_buttons = [self.buttons[pos] for pos in typing_input]
        for b in pressed_buttons:
            b.pressed += 1
        return ''.join([b.key for b in pressed_buttons])
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def string_maker(link):
        if link == Link.empty:
            return empty_repr
        else:
            return front + str(link.first) + mid + make_to_string(front, mid, back, empty_repr)(link.rest) + back
    return string_maker
