...
def merge(lst1, lst2):
    ...
    if not lst1 or not lst2:
        ...
        return lst2[0] + merge(list1, lst2[1:])
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    for cur_row in range(max_rows - 1, -1, -1):
        if get_piece(board, cur_row, column) == '-':
            new_row = replace_elem(board[cur_row], column, player)
            return cur_row, replace_elem(board, cur_row, new_row)
    return -1, board
...
def print_board(board, max_rows, max_cols):
    ...
    """
    "*** YOUR CODE HERE ***"
    for cur_row in range(max_rows):
        print(' '.join(str(piece) for piece in board[cur_row]))
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    "*** YOUR CODE HERE ***"
    cur_row = board[row]
    win_lst = [player for _ in range(num_connect)]
    for i in range(max_cols - num_connect + 1):
        if cur_row[i:i+num_connect] == win_lst:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    cur_col = []
    for cur_row in range(max_rows):
        cur_col += board[cur_row][col]
    win_lst = [player for _ in range(num_connect)]
    for i in range(max_cols - num_connect + 1):
        if cur_col[i:i+num_connect] == win_lst:
            return True
    return False
