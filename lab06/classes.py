...
class Player(object):
    ...
    def go_to(self, location):
        ...
        print('You are at {p}'.format(p=self.place.name))
    ...
    def talk_to(self, person):
        ...
        # "*** YOUR CODE HERE ***"
            ...
            print('{p} says: {m}'.format(p=person, m=self.place.characters[person].talk()))
        else:
            print('{p} is not here.'.format(p=person))
    ...
    def take(self, thing):
        ...
        # "*** YOUR CODE HERE ***"
            ...
            print('Player takes the {t}'.format(t=thing))
        else:
            print('{t} is not here.'.format(t=thing))
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        "*** YOUR CODE HERE ***"
        if key and place in self.place.exits:
            key.use(self.place.exits[place][0])
        else:
            print('{p} can\'t be unlocked without a key!'.format(p=place))
...
class Key(Thing):
    def use(self, place):
        if not place.locked:
            print('{p} is already unlocked!'.format(p=place.name))
        else:
            place.locked = False
            print('{p} is now unlocked!'.format(p=place.name))
